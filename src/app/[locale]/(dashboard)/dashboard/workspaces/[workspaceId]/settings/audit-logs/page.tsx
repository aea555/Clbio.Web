...
import { useTranslations } from "next-intl"; ///settings/audit-logs/page.tsx]
...
export default function WorkspaceAuditLogsPage() {
  const t = useTranslations("AuditLogs"); ///settings/audit-logs/page.tsx]
  ...
  // Helper to color-code actions (Logic preserved, labels translated via t())
  ...
  // Helper for translating action label without breaking the switch color logic
  const getActionLabel = (action: string) => {
    const key = action?.toLowerCase();
    if (["create", "update", "delete"].includes(key)) {
      return t(`table.actions.${key}`);
    }
    return action || t("table.actions.other");
  };
    ...
    <div className="max-w-5xl mx-auto w-full">
      ...
      <div className="bg-white dark:bg-[#1a2430] rounded-xl border border-[#e8edf3] dark:border-[#2d3a4a] overflow-hidden shadow-sm flex flex-col min-h-[500px] mt-6">
        ...
        <div className="px-6 py-4 border-b border-[#e8edf3] dark:border-[#2d3a4a] flex justify-between items-center bg-[#f8fafb] dark:bg-[#111921]">
          <h3 className="font-bold text-[#0e141b] dark:text-[#e8edf3]">{t("log_title")}</h3>
          <div className="flex items-center gap-2">
            {isFetching && <span className="text-xs text-[#4c99e6]">{t("refreshing")}</span>}
            {result &&
              <span className="text-xs text-[#507395] dark:text-[#94a3b8]">
                {t("pagination.page_info", { page: result.meta.page, total: totalPages })}
              </span>
            }
          </div>
        </div>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="bg-white dark:bg-[#1a2430] border-b border-[#e8edf3] dark:border-[#2d3a4a]">
                  <th className="px-6 py-3 font-semibold text-[#0e141b] dark:text-[#e8edf3]">{t("table.headers.actor")}</th>
                  <th className="px-6 py-3 font-semibold text-[#0e141b] dark:text-[#e8edf3]">{t("table.headers.action")}</th>
                  <th className="px-6 py-3 font-semibold text-[#0e141b] dark:text-[#e8edf3]">{t("table.headers.details")}</th>
                  <th className="px-6 py-3 font-semibold text-[#0e141b] dark:text-[#e8edf3]">{t("table.headers.entity")}</th>
                  <th className="px-6 py-3 text-right font-semibold text-[#0e141b] dark:text-[#e8edf3]">{t("table.headers.date")}</th>
                </tr>
              </thead>
        ...
        {result && totalPages > 1 && (
          <div className="px-6 py-4 border-t border-[#e8edf3] dark:border-[#2d3a4a] bg-white dark:bg-[#1a2430] flex items-center justify-between">
            <p className="text-sm text-[#507395] dark:text-[#94a3b8]">
              {t("pagination.results_info", {
                start: (result.meta.page - 1) * result.meta.pageSize + 1,
                end: Math.min(result.meta.page * result.meta.pageSize, result.meta.totalCount),
