...
import { useTranslations } from "next-intl"; ///boards/[boardId]/tasks/[taskId]/page.tsx]
...
export default function TaskDetailPage() {
    const t = useTranslations("TaskDetail"); ///boards/[boardId]/tasks/[taskId]/page.tsx]
    ...
    const renderDate = (dateStr: string | undefined) => {
        ...
        return isValid(date) ? format(date, "MMM d, yyyy") : t("invalid_date");
    };
    ...
    return (
        <div className="min-h-full bg-background flex flex-col relative overflow-hidden transition-colors duration-300">
            ...
            <div className="p-4 md:p-8 flex-1 overflow-y-auto custom-scrollbar relative z-10">
                <ConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={handleDelete} title={t("modals.delete_task_title")} description={t("modals.delete_task_desc")} confirmText={t("modals.delete_task_confirm")} variant="danger" isLoading={deleteTask.isPending} />

                <ConfirmationModal
                    isOpen={!!attachmentToDelete}
                    onClose={() => setAttachmentToDelete(null)}
                    onConfirm={() => { if (attachmentToDelete) { deleteAttachment.mutate(attachmentToDelete.id); setAttachmentToDelete(null); } }}
                    title={t("modals.delete_file_title")}
                    description={t("modals.delete_file_desc", { name: attachmentToDelete?.fileName ?? "" })}
                    confirmText={t("modals.delete_file_confirm")}
                    variant="danger"
                ...
                <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
                    <div className="lg:col-span-3 space-y-8">
                        <div className="bg-card rounded-xl border border-border-base p-6 shadow-sm">
                            ...
                            <div className="mt-2 flex items-center gap-4 text-xs text-muted-foreground">
                                <span>{t("created_at", { date: renderDate(task?.createdAt) })}</span>
                                <span>•</span>
                                <span className="font-medium bg-background px-2 py-0.5 rounded border border-border-base">
                                    {t("in_list")} <span className="text-foreground">
                        ...
                        <div className="bg-card rounded-xl border border-border-base p-6 shadow-sm">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h3 className="font-bold text-foreground">{t("description")}</h3>
                            </div>
                        ...
                        <div className="bg-card rounded-xl border border-border-base p-6 shadow-sm">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="font-bold text-foreground">{t("attachments")}</h3>
                                </div>
                            ...
                            {attachments && attachments.length > 0 ? (
                                        ...
                                        return (
                                            <div key={file.id} className="flex items-center gap-3 p-3 rounded-lg border border-border-base hover:bg-background transition-colors group relative bg-card">
                                                ...
                                                <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-card shadow-md rounded border border-border-base">
                                                    <button onClick={(e) => handleDownload(e, file)} className="flex items-center justify-center hover:cursor-pointer p-1 text-muted-foreground hover:text-primary transition-colors">
                                                        <span className="material-symbols-outlined text-[18px]">download</span>
                                                    </button>
                                                    {canDeleteAttachment(file) && (
                                                        <button onClick={() => setAttachmentToDelete(file)} className="flex items-center justify-center hover:cursor-pointer p-1 text-muted-foreground hover:text-red-500 transition-colors">
                                ...
                                <p className="text-sm text-muted-foreground italic">{t("no_attachments")}</p>
                            )}
                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-card rounded-xl border border-border-base p-4 shadow-sm">
                            <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">{t("status")}</h4>
                            <select
                            ...
                            >
                                <option value={TaskProgressStatus.Assigned}>{t("status_options.todo")}</option>
                                <option value={TaskProgressStatus.InProgress}>{t("status_options.in_progress")}</option>
                                <option value={TaskProgressStatus.ReadyForReview}>{t("status_options.review")}</option>
                                <option value={TaskProgressStatus.Received}>{t("status_options.done")}</option>
                            </select>
                        </div>

                        <div className="bg-card rounded-xl border border-border-base p-4 shadow-sm space-y-4">
                            <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t("details")}</h4>
                            ...
                            <div>
                                ...
                                <span className="text-sm text-foreground italic">{t("no_date_set")}</span>
                            </div>
                        </div>
