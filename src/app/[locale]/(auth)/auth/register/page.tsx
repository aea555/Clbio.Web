...
export default function RegisterPage() {
  ...
  const registerFormSchema = registerSchema
    .extend({
      confirmPassword: z.string().min(1, t("errors.confirm_required")),
    })
  ...
  const handleGoogleSuccess = async (credentialResponse: CredentialResponse) => {
    if (!credentialResponse.credential) {
      toast.error(t("toasts.google_signup_fail"));
      return;
    }
      ...
      { idToken: credentialResponse.credential },
      {
        onSuccess: async (tokenResponse: any) => {
          ...
          try {
            ...
            toast.success(t("toasts.welcome_created", { name: userData.displayName }));
            router.push("/dashboard");
          } catch (err) {
            console.error("Profile fetch failed", err);
            toast.error(t("toasts.profile_load_fail"));
          }
        },
        onError: (error: any) => {
    ...
    registerMutation.mutate(payload, {
      onSuccess: (response: ApiResponse<ReadUserDto>) => {
        ...
        if (!responseData || !response.success) {
          toast.error(t("toasts.registration_failed"));
          return;
        }
        ...
        if (id && email) {
          ...
          toast.success(t("toasts.verify_email"));
      ...
      onError: (error: any) => {
        ...
        if (msg.includes("verify") || msg.includes("exists")) {
          ...
          toast.info(t("toasts.account_exists"));
          router.push("/auth/verify-email");
          return;
        }
        toast.error(msg);
    ...
    <GoogleOAuthProvider clientId={process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || ""}>
      <div className="bg-[#f6f7f8] dark:bg-[#111921] min-h-screen flex flex-col font-sans">
        ...
        <main className="flex-grow flex items-center justify-center p-4">
          <div className="w-full max-w-[480px] bg-white dark:bg-[#1a242f] rounded-xl shadow-lg border border-[#e8edf3] dark:border-[#2d3a26] p-8 md:p-10">
            ...
            <div className="flex justify-center mb-6 w-full">
              <GoogleLogin
                ...
                onError={() => toast.error(t("toasts.google_fail"))}
                theme="outline"
                size="large"
                width="300"
                text="signup_with"
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <button
                ...
                className="w-full hover:cursor-pointer flex items-center justify-center rounded-lg bg-[#4c99e6] py-3 px-4 text-sm font-bold text-white shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-[#4c99e6] focus:ring-offset-2 transition-colors mt-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
