...
export default function LoginPage() {
  ...
  const handleLoginSuccess = async (tokenResponse: any) => {
    ...
    try {
      ...
      toast.success(t("toasts.welcome", { name: userData.displayName }));
      router.push("/dashboard");
    } catch (err) {
      console.error("Profile fetch failed during login", err);
      toast.error(t("toasts.profile_fail"));
    }
  };
    ...
    loginMutation.mutate(data, {
      ...
      onError: (error: any) => {
        ...
        if (msg.toLowerCase().includes("verify") || msg.toLowerCase().includes("not verified")) {
          ...
          toast.error(t("toasts.verification_required"));
  ...
  const handleGoogleSuccess = (credentialResponse: CredentialResponse) => {
    if (!credentialResponse.credential) {
      toast.error(t("toasts.google_credential_fail"));
      return;
    }
        ...
        <main className="flex-grow flex items-center justify-center p-4">
          <div className="w-full max-w-[480px] bg-white dark:bg-[#1a242f] rounded-xl shadow-lg border border-[#e8edf3] dark:border-[#2d3a46] p-8 md:p-10">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              >
                {loginMutation.isPending ? t("signing_in") : t("signin_button")}
              </button>
            </form>
          </div>
        </main>
