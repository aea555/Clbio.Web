...
import { useWorkspacePermissions } from "@/hooks/use-workspace-permissions"; // Import Hook
...
export default function WorkspaceGeneralSettingsPage() {
  ...
  // Use new permissions hook
  const { isArchived, canEdit } = useWorkspacePermissions(workspaceId);
  ...
  const { updateWorkspace, deleteWorkspace, archiveWorkspace, unarchiveWorkspace } = useWorkspaceMutations();
      ...
      <div className="max-w-4xl mx-auto w-full">
        ...
        <div className="space-y-8">
          ...
          <section className={`bg-white dark:bg-[#1a2430] rounded-xl border border-[#e8edf3] dark:border-[#2d3a4a] p-6 shadow-sm transition-opacity duration-300 ${isArchived ? "bg-gray-50/50" : ""}`}>
             {/* REMOVED: The blocking overlay div */}
             ...
             <form onSubmit={handleSubmit(onUpdate)} className="space-y-5">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="space-y-1.5">
                   ...
                   <input
                     {...register("name")}
                     // FIX: Use disabled but allow selection (browser default behavior for disabled input)
                     disabled={!canEdit} 
                     id="name"
                     type="text"
               ...
               {/* FIX: Hide Save button if Read Only */}
               {canEdit && (
